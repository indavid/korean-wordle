...
class GameBoard {
    constructor(){
        ...
        this.board = Array.from(Array(this.rows), ()=>new Array(this.cols).fill(null));
        this.styleBoard = Array.from(Array(this.rows), ()=>new Array(this.cols).fill(null));
    }
...
class GameController {
    ...
    convertToKorean(letter) {
        ...
        if (englishToKorean[letter.toLowerCase()]) return englishToKorean[letter.toLowerCase()];
    }
    // Add letter to gameBoard array
    addLetter(letter) {
        if (this.currentGuess.length < this.answerLength) this.currentGuess += letter;
        else this.currentGuess = this.currentGuess.substring(0, this.answerLength - 1) + letter;
        this.gameBoard.addLetter(this.currentRound, this.currentGuess.length - 1, letter);
        this.gameBoard.addStyle(this.currentRound, this.currentGuess.length - 1, "filled");
    }
    ...
    verifyGuess() {
        ...
        for(let i = 0; i < this.answerLength; i++)if (guessParts[i] === answerParts[i]) {
        ...
        if (this.isGameWon) {
            ...
            this.currentRound = 6;
        } else if (this.currentRound === this.rounds) this.isGameRunning = false;
    }
    // Handle game logic of keyboard event from user
    handleEvent(key, preventDefault) {
        if (key === "Backspace" || key === "Delete") this.deleteLetter();
        else if (key === "Enter" || key === "ENTER") this.verifyGuess();
        else if (this.isLetter(key) || this.isKorean(key)) {
            if (this.isLetter(key)) key = this.convertToKorean(key);
            this.addLetter(key);
        } else preventDefault;
    }
}
class ScreenController {
    ...
    setLoading(isLoading) {
        isLoading ? this.spinner.style.visibility = "visible" : this.spinner.style.visibility = "hidden";
    }
    ...
    renderBoard() {
        ...
        for(let i = 0; i < board.length; i++){
            ...
            for(let j = 0; j < board[i].length; j++){
                ...
                if (letterDiv.innerText) letterDiv.classList.add(styleBoard[i][j]);
            }
        }
    }
    ...
    listen() {
        this.body.addEventListener("keydown", ({ key , preventDefault  })=>{
            ...
            this.renderBoard();
            console.log(this.gameController.getIsGameRunning() + " and " + this.gameController.getIsGameWon());
            if (!this.gameController.getIsGameRunning()) {
                if (this.gameController.getIsGameWon()) setTimeout(()=>alert("You won! :)"), 500);
                else setTimeout(()=>alert("You lose. :("), 500);
            }
        });
